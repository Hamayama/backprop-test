...
;;
;; ＜変更点＞
;;   活性化関数を ReLU 関数にし、中間層の数を1増やし、中間層のニューロン数も3増やした
;;
...
;; メイン処理
(define (main args)
  (define ml1 (make <middle-layer>))
  (define ml2 (make <middle-layer>))
  (define ol  (make <output-layer>))

  ;; 各層の初期化
  (middle-layer-init ml1 n-in  n-mid)
  (middle-layer-init ml2 n-mid n-mid)
    ...
    (let ((index-random (shuffle (iota n-data)))
      ...
      (dolist (idx index-random)
        (let ((x (f64array-ref input-data   0 idx))
              ...
              (y #f))
          ;; 順伝播
          (middle-layer-forward  ml1 (f64array-simple 0 1 0 1 x))
          (middle-layer-forward  ml2 (slot-ref ml1 'y))
          (output-layer-forward  ol  (slot-ref ml2 'y))
          ;; 逆伝播 (ouput -> middle の順なので注意)
          (output-layer-backward ol  (f64array-simple 0 1 0 1 t))
          (middle-layer-backward ml2 (slot-ref ol  'grad-x))
          (middle-layer-backward ml1 (slot-ref ml2 'grad-x))
          ;; 重みとバイアスの更新
          (middle-layer-update   ml1 eta)
          (middle-layer-update   ml2 eta)
  ...
  ;; 最終パラメータの表示
  ;(print "ml1.w = "(slot-ref ml1 'w))
  ;(print "ml1.b = "(slot-ref ml1 'b))
  ;(print "ml2.w = "(slot-ref ml2 'w))
  ;(print "ml2.b = "(slot-ref ml2 'b))
